...
INSERT INTO `defterp`.`account` (name, title, active,type, code) 
...
INSERT INTO `defterp`.`journal` (name, code, type, active) 
...
INSERT INTO `defterp`.`tax`  (name, amount, type_tax_use, active, percent)
VALUES  ('Sales 15%',0.15,'Sale',1,15),
        ('Purchases 20%',0.2,'Purchase',1,20);

INSERT INTO `defterp`.`user`  (login, password, name, user_type, active)
VALUES ('user','user','User','User', 1);




CREATE TABLE IF NOT EXISTS `defterp`.`tax` (
...
CREATE TABLE IF NOT EXISTS `defterp`.`account` (
...
CREATE TABLE IF NOT EXISTS `defterp`.`product_category` (
  `id` INT NOT NULL AUTO_INCREMENT,
...
CREATE TABLE IF NOT EXISTS `defterp`.`user` (
  `id` INT NOT NULL AUTO_INCREMENT,
 ...
 CREATE TABLE IF NOT EXISTS `defterp`.`product_uom_category` (
  `id` INT NOT NULL AUTO_INCREMENT,
  `name` VARCHAR(64) NOT NULL,
  ...
  CREATE TABLE IF NOT EXISTS `defterp`.`product_uom` (
  `id` INT NOT NULL AUTO_INCREMENT,
  `name` VARCHAR(64) NOT NULL,
  ...
  CONSTRAINT `product_uom_category_id_fkey` 
    ...
    REFERENCES `defterp`.`product_uom_category`(`id`));



CREATE TABLE IF NOT EXISTS `defterp`.`product` (
  `id` INT NOT NULL AUTO_INCREMENT,
  `image` LONGBLOB NULL,
  ...
  CONSTRAINT `product_template_categ_id_fkey`
    ...
    REFERENCES `defterp`.`product_category` (`id`),
  CONSTRAINT `product_template_uom_id_fkey`
    FOREIGN KEY (`uom_id`)
    REFERENCES `defterp`.`product_uom` (`id`));

    
    
CREATE TABLE IF NOT EXISTS `defterp`.`partner` (
  `id` INT NOT NULL AUTO_INCREMENT,
  ...
  CONSTRAINT `partner_accountReceivable_id_fkey`
    ...
    REFERENCES `defterp`.`account` (`id`),
  CONSTRAINT `partner_accountPayable_id_fkey`
    FOREIGN KEY (`accountPayable_id`)
    REFERENCES `defterp`.`account` (`id`));
    
    
CREATE TABLE IF NOT EXISTS `defterp`.`sale_order` (
  `id` INT NOT NULL AUTO_INCREMENT,
  ...
  CONSTRAINT `sale_order_partner_id_fkey`
    ...
    REFERENCES `defterp`.`partner` (`id`));
    
  
CREATE TABLE IF NOT EXISTS `defterp`.`sale_order_line` (
  `id` INT NOT NULL AUTO_INCREMENT,
  `date` DATETIME NULL,
  ...
  CONSTRAINT `sale_order_line_order_id_fkey`
    ...
    REFERENCES `defterp`.`sale_order` (`id`),
  CONSTRAINT `sale_order_line_product_id_fkey`
    FOREIGN KEY (`product_id`)
    REFERENCES `defterp`.`product` (`id`),
  CONSTRAINT `sale_order_line_tax_id_fkey`
    FOREIGN KEY (`tax_id`)
    REFERENCES `defterp`.`tax` (`id`));

   
CREATE TABLE IF NOT EXISTS `defterp`.`journal` (
  `id` INT NOT NULL AUTO_INCREMENT,
  `name` VARCHAR(64) NOT NULL,
  `code` VARCHAR(64) NOT NULL,
  `type` varchar(64) NOT NULL,
...
CREATE TABLE IF NOT EXISTS `defterp`.`purchase_order` (
...
CREATE TABLE IF NOT EXISTS `defterp`.`journal_entry` (
  `id` INT NOT NULL AUTO_INCREMENT,
  `journal_id` INT NULL,
  `name` VARCHAR(64) NULL,
  `ref` VARCHAR(64) NULL,
  ...
  CONSTRAINT `journal_entry_journal_id_fkey`
    ...
    REFERENCES `defterp`.`journal` (`id`),
  CONSTRAINT `journal_entry_partner_id_fkey`
    FOREIGN KEY (`partner_id`)
    REFERENCES `defterp`.`partner` (`id`));    
 
 
CREATE TABLE IF NOT EXISTS `defterp`.`journal_item` (
  ...
  CONSTRAINT `journal_entry_line_entry_id_fkey`
    ...
    REFERENCES `defterp`.`journal_entry` (`id`),
...
CREATE TABLE IF NOT EXISTS `defterp`.`inventory` (
  `id` INT NOT NULL AUTO_INCREMENT,
  `max_qty` DOUBLE NULL,
  `min_qty` DOUBLE NULL,
  `quantity` DOUBLE NOT NULL,
...
CREATE TABLE IF NOT EXISTS `defterp`.`invoice` (
  ...
  CONSTRAINT `invoice_purchase_id_fkey`
    ...
    REFERENCES `defterp`.`purchase_order` (`id`),
...
CREATE TABLE IF NOT EXISTS `defterp`.`payment`  (
...
CREATE TABLE IF NOT EXISTS `defterp`.`purchase_order_line` (
...
CREATE TABLE IF NOT EXISTS `defterp`.`invoice_line` (
...
CREATE TABLE IF NOT EXISTS `defterp`.`delivery_order` (
  `id` INT NOT NULL AUTO_INCREMENT,
  `partner_id` INT NOT NULL,
  ...
  CONSTRAINT `delivery_order_back_order_id_fkey`
    ...
    REFERENCES `defterp`.`delivery_order` (`id`),
...
CREATE TABLE IF NOT EXISTS `defterp`.`delivery_order_line` (
 ...
 CREATE TABLE IF NOT EXISTS `defterp`.`invoice_tax` (
  `id` INT NOT NULL AUTO_INCREMENT,
  `date` DATETIME NULL,
  `tax_amount` DOUBLE NULL,
  `account_id` INT NULL,
...
CREATE TABLE IF NOT EXISTS `defterp`.`invoice_payment` (
  `id` INT NOT NULL AUTO_INCREMENT,
  `invoice_id` INT NOT NULL,
